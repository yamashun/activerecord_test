...
class TokensControllerTest < ActionController::TestCase
  ...
  test "should create token" do
    assert_difference('Token.count') do
      post :create, token: { car_id: @token.car_id, expiration_date: @token.expiration_date, status: @token.status, token: @token.token }
    end
  ...
  test "should update token" do
    patch :update, id: @token, token: { car_id: @token.car_id, expiration_date: @token.expiration_date, status: @token.status, token: @token.token }
    assert_redirected_to token_path(assigns(:token))
  end
